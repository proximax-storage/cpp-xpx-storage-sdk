...
class IsFileRequestContext
        : public RequestContext, public std::enable_shared_from_this<IsFileRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::IsFileResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    IsFileRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestIsFile( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<PathIsFileResponse>& response );
};
