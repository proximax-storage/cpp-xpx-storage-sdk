...
class OpenFileRequestContext
        : public RequestContext, public std::enable_shared_from_this<OpenFileRequestContext>
{
...
private:
    ...
    storage::OpenFileRequest m_request;
    grpc::ServerAsyncResponseWriter<storage::OpenFileResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    OpenFileRequestContext( storage::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestOpenFile( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<OpenFileResponse>& response );
};
