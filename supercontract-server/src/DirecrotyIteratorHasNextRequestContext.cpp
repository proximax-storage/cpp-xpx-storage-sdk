...
DirectoryIteratorHasNextRequestContext::DirectoryIteratorHasNextRequestContext(
...
void DirectoryIteratorHasNextRequestContext::processRequest()
{
    ...
    FolderIteratorHasNextRequest request;
    ...
    executor->folderIteratorHasNext( driveKey, request );
}

void DirectoryIteratorHasNextRequestContext::onCallExecuted( const std::optional<FolderIteratorHasNextResponse>& response )
{
    if ( !*m_serviceIsActive )
    {
        return;
    ...
    storageServer::DirectoryIteratorHasNextResponse msg;
    grpc::Status status;
    if ( response )
    {
        msg.set_has_next( response->m_hasNext );
    } else
    {
