...
class DirectoryIteratorNextRequestContext
        : public RequestContext, public std::enable_shared_from_this<DirectoryIteratorNextRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::DirectoryIteratorNextResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    DirectoryIteratorNextRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestDirectoryIteratorNext( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<FolderIteratorNextResponse>& response );
};
