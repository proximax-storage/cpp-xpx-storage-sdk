...
class PathExistRequestContext
        : public RequestContext, public std::enable_shared_from_this<PathExistRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::PathExistResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    PathExistRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestPathExist( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<PathExistResponse>& response );
};
