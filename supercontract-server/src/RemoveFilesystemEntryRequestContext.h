...
class RemoveFilesystemEntryRequestContext
        : public RequestContext, public std::enable_shared_from_this<RemoveFilesystemEntryRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::RemoveFilesystemEntryResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    RemoveFilesystemEntryRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestRemoveFilesystemEntry( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<RemoveFilesystemEntryResponse>& response );
};
