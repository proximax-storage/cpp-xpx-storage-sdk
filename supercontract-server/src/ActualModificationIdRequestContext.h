...
class ActualModificationIdRequestContext
        : public RequestContext, public std::enable_shared_from_this<ActualModificationIdRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::ActualModificationIdResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    ActualModificationIdRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestGetActualModificationId( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<ActualModificationIdResponse>& response );
};
