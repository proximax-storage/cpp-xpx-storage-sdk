...
class FileSizeRequestContext
        : public RequestContext, public std::enable_shared_from_this<FileSizeRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::FileSizeResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    FileSizeRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestFileSize( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<FileSizeResponse>& response );
};
