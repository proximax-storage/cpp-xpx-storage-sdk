...
CreateDirectoriesRequestContext::CreateDirectoriesRequestContext(
...
void CreateDirectoriesRequestContext::processRequest()
{
    ...
    CreateDirectoriesRequest request;
    ...
    executor->createDirectories( driveKey, request );
}

void CreateDirectoriesRequestContext::onCallExecuted( const std::optional<CreateDirectoriesResponse>& response )
{
    if ( !*m_serviceIsActive )
    {
        return;
    ...
    storageServer::CreateDirectoriesResponse msg;
    grpc::Status status;
    if ( response )
    {
