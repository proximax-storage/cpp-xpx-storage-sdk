...
SynchronizeStorageRequestContext::SynchronizeStorageRequestContext(
...
void SynchronizeStorageRequestContext::processRequest()
{
    ...
    request.m_rootHash = Hash256( *reinterpret_cast<const std::array<uint8_t, 32>*>(m_request.storage_hash().data()));
    request.m_callback = [pThis = shared_from_this()]( auto response )
    {
        pThis->onCallExecuted( response );
    };

    executor->manualSynchronize( driveKey, request );
}

void SynchronizeStorageRequestContext::onCallExecuted( const std::optional<SynchronizationResponse>& response )
{
    if ( !*m_serviceIsActive )
    {
        return;
    ...
    storageServer::SynchronizeStorageResponse msg;
    grpc::Status status;
    if ( response )
    {
        msg.set_status( response->success );
    } else
    {
