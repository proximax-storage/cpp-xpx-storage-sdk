...
class DirectoryIteratorHasNextRequestContext
        : public RequestContext, public std::enable_shared_from_this<DirectoryIteratorHasNextRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::DirectoryIteratorHasNextResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    DirectoryIteratorHasNextRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestDirectoryIteratorHasNext( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<FolderIteratorHasNextResponse>& response );
};
