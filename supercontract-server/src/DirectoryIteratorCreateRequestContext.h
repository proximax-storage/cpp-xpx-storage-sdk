...
class DirectoryIteratorCreateRequestContext
        : public RequestContext, public std::enable_shared_from_this<DirectoryIteratorCreateRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::DirectoryIteratorCreateResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    DirectoryIteratorCreateRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestDirectoryIteratorCreate( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<FolderIteratorCreateResponse>& response );
};
