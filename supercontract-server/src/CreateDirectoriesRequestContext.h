...
class CreateDirectoriesRequestContext
        : public RequestContext, public std::enable_shared_from_this<CreateDirectoriesRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::CreateDirectoriesResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    CreateDirectoriesRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestCreateDirectories( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<CreateDirectoriesResponse>& response );
};
