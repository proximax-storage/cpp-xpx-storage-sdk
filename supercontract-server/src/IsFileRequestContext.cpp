...
IsFileRequestContext::IsFileRequestContext(
...
void IsFileRequestContext::processRequest()
{
    ...
    executor->pathIsFile( driveKey, request );
}

void IsFileRequestContext::onCallExecuted( const std::optional<PathIsFileResponse>& response )
{
    if ( !*m_serviceIsActive )
    {
        return;
    ...
    storageServer::IsFileResponse msg;
    grpc::Status status;
    if ( response )
    {
        msg.set_is_file( response->m_isFile );
    } else
    {
