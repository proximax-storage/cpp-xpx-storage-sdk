...
class WriteFileRequestContext
        : public RequestContext, public std::enable_shared_from_this<WriteFileRequestContext>
{
...
private:
    ...
    storage::WriteFileRequest m_request;
    grpc::ServerAsyncResponseWriter<storage::WriteFileResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    WriteFileRequestContext( storage::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestWriteFile( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<WriteFileResponse>& response );
};
