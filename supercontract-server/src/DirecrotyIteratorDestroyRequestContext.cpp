...
DirectoryIteratorDestroyRequestContext::DirectoryIteratorDestroyRequestContext(
...
void DirectoryIteratorDestroyRequestContext::processRequest()
{
    ...
    FolderIteratorDestroyRequest request;
    ...
    executor->folderIteratorDestroy( driveKey, request );
}

void
DirectoryIteratorDestroyRequestContext::onCallExecuted( const std::optional<FolderIteratorDestroyResponse>& response )
{
    if ( !*m_serviceIsActive )
    {
        return;
    ...
    storageServer::DirectoryIteratorDestroyResponse msg;
    grpc::Status status;
    if ( response )
    {
