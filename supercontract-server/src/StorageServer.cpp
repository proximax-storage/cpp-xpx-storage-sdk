...
namespace sirius::drive::contract {
    ...
    void StorageServer::registerEvaluateStorageHash() {
        ...
        auto synchronizeContext = std::make_shared<EvaluateStorageHashRequestContext>(m_service, *m_cq,
                                                                                      m_serviceIsActive,
                                                                                      m_executor);
        auto *tag = new AcceptRequestRPCTag(synchronizeContext, [this, serviceIsActive = m_serviceIsActive] {
            if (!*serviceIsActive) {
    ...
    void StorageServer::registerFlush() {
        ...
        auto synchronizeContext = std::make_shared<FlushRequestContext>(m_service, *m_cq, m_serviceIsActive,
                                                                        m_executor);
        auto *tag = new AcceptRequestRPCTag(synchronizeContext, [this, serviceIsActive = m_serviceIsActive] {
            if (!*serviceIsActive) {
                return;
    ...
    void StorageServer::registerGetActualModificationId() {
        ...
        auto context = std::make_shared<ActualModificationIdRequestContext>(m_service, *m_cq, m_serviceIsActive,
                                                                            m_executor);
        auto *tag = new AcceptRequestRPCTag(context, [this, serviceIsActive = m_serviceIsActive] {
            if (!*serviceIsActive) {
                return;
    ...
    void StorageServer::registerCreateDirectories() {
        ...
        auto context = std::make_shared<CreateDirectoriesRequestContext>(m_service, *m_cq, m_serviceIsActive,
                                                                         m_executor);
        auto *tag = new AcceptRequestRPCTag(context, [this, serviceIsActive = m_serviceIsActive] {
            if (!*serviceIsActive) {
                return;
