...
RemoveFilesystemEntryRequestContext::RemoveFilesystemEntryRequestContext(
...
void RemoveFilesystemEntryRequestContext::processRequest()
{
    ...
    RemoveFilesystemEntryRequest request;
    ...
    executor->removeFsTreeEntry( driveKey, request );
}

void RemoveFilesystemEntryRequestContext::onCallExecuted( const std::optional<RemoveFilesystemEntryResponse>& response )
{
    if ( !*m_serviceIsActive )
    {
        return;
    ...
    storageServer::RemoveFilesystemEntryResponse msg;
    grpc::Status status;
    if ( response )
    {
