...
class MoveFilesystemEntryRequestContext
        : public RequestContext, public std::enable_shared_from_this<MoveFilesystemEntryRequestContext>
{
...
private:
    ...
    grpc::ServerAsyncResponseWriter<storageServer::MoveFilesystemEntryResponse> m_responder;

    std::weak_ptr<ModificationsExecutor> m_executor;

public:

    MoveFilesystemEntryRequestContext( storageServer::StorageServer::AsyncService& service,
    ...
    {
        m_service.RequestMoveFilesystemEntry( &m_context, &m_request, &m_responder, &m_completionQueue,
...
private:
    ...
    void onCallExecuted( const std::optional<MoveFilesystemEntryResponse>& response );
};
