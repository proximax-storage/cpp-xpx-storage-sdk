...
#define STACK_FRAMES_BUFFERSIZE (int)100 * 1024

static void * STACK_FRAMES_BUFFER[100 * 1024];
static void * OFFSET_FRAMES_BUFFER[100 * 1024];
...
{
    static const char errorString[] = "Offset cannot be resolved: No offset present?\n\0?";
    static char       printArray[100 * 1024] = {0};
...
static void Addr2LinePrint(void const * const addr)
{
    char addr2lineCmd[2 * 4096] = {0};

    //have addr2line map the address to the relent line in the code
    (void)sprintf(addr2lineCmd,"addr2line -C -i -f -p -s -a -e %s %p ", (char *)gExecutablePath, addr);
