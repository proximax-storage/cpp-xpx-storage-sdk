...
//                {
                    ...
                    //gReplicator3->asyncApprovalTransactionHasBeenPublished( PublishedModificationApprovalTransactionInfo(*MyReplicatorEventHandler::m_approvalTransactionInfo) );
                      gReplicator3->asyncApprovalTransactionHasBeenPublished( std::make_unique<PublishedModificationApprovalTransactionInfo>(MyReplicatorEventHandler::m_approvalTransactionInfo.value()) );
//                } );
            }
            else
            {
...
//        actionList.push_back( Action::upload( clientFolder / "a.txt", "fff0/a.txt" ) );
    ...
    if ( gRestartReplicators )
    {
        ...
        gReplicator->shutdownReplicator();
        gReplicator.reset();
        usleep(10000);
        gReplicator2->shutdownReplicator();
        gReplicator2.reset();
        usleep(10000);
        gReplicator3->shutdownReplicator();
        gReplicator3.reset();
...
//    replicator->asyncAddDrive( DRIVE_PUB_KEY, AddDriveRequest{100,         0, replicatorList, clientKeyPair.publicKey(), replicatorList, replicatorList } );
    replicator->asyncAddDrive( DRIVE_PUB_KEY, std::unique_ptr<AddDriveRequest>( new AddDriveRequest{100*1024*1024,0,{},replicatorList,clientKeyPair.publicKey(),{},{} } ) );

    replicator->asyncAddDownloadChannelInfo( DRIVE_PUB_KEY, std::unique_ptr<DownloadRequest>( new DownloadRequest{ downloadChannelHash1.array(), 1024*1024, replicatorList, { clientKeyPair.publicKey(), clientKeyPair1.publicKey() }} ));
    replicator->asyncAddDownloadChannelInfo( DRIVE_PUB_KEY, std::unique_ptr<DownloadRequest>( new DownloadRequest{ downloadChannelHash2.array(), 10*1024*1024, replicatorList, { clientKeyPair.publicKey(), clientKeyPair1.publicKey() }} ));
    replicator->asyncAddDownloadChannelInfo( DRIVE_PUB_KEY, std::unique_ptr<DownloadRequest>( new DownloadRequest{ downloadChannelHash3.array(), 1024*1024, replicatorList, { clientKeyPair.publicKey(), clientKeyPair1.publicKey() }} ));
...
static void modifyDrive( std::shared_ptr<Replicator>    replicator,
                         ...
                         uint64_t                       maxDataSize )
{
    replicator->asyncModify( DRIVE_PUB_KEY, std::unique_ptr<ModificationRequest>( new ModificationRequest{
                                                                                        clientDataInfoHash,
                                                                                        transactionHash,
                                                                                        maxDataSize,
                                                                                        replicatorList } ) );
                               ...
                               const sirius::Hash256& transactionHash )
{
    ...
    std::error_code ec;
    InfoHash hash = gClientSession->addActionListToSession(  actionList,
                                                           DRIVE_PUB_KEY,
                                                           replicatorList,
                                                           tmpFolder,
                                                           totalModifySize, {}, ec );
