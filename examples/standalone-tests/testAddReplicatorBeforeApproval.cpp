...
#define TEST_NAME AddReplicatorBeforeApproval
...
public:
    ...
    {
        if ( transactionInfo.m_opinions[0].m_replicatorKey != m_addrList.back().array() )
        {
            return;
        }
...
TEST( ModificationTest, TEST_NAME )
{
    ...
    env.waitRootHashCalculated( client.m_modificationTransactionHashes[0], NUMBER_OF_REPLICATORS - 1 );
