                        ...
                        bool        startReplicator = true)
    {
        if (true) {
            for( int i=1; i<=numberOfReplicators; i++ )
            {
                ...
                boost::asio::ip::tcp::endpoint point = {boost::asio::ip::address::from_string(ipAddr), static_cast<ushort>(port)};
                auto publicKey = replicatorKeyPair.publicKey();
                ...
                m_addrList.emplace_back( ReplicatorInfo{ point, publicKey } );
                m_threads.push_back({});
            }
        }
    }
...
#pragma mark --ReplicatorEventHandler methods and variables
    ...
    inline static std::optional<ApprovalTransactionInfo>           m_approvalTransactionInfo;
    inline static std::optional<DownloadApprovalTransactionInfo>   m_dnApprovalTransactionInfo;
    inline static std::mutex                                       m_transactionInfoMutex;
    ...
    {
        const std::unique_lock<std::mutex> lock(TestEnvironment::m_transactionInfoMutex);
...
//        }
    ...
    virtual void modifyApprovalTransactionIsReady( Replicator& replicator, ApprovalTransactionInfo&& transactionInfo )  override
    {
        ...
        if ( !TestEnvironment::m_approvalTransactionInfo )
        {
            TestEnvironment::m_approvalTransactionInfo = { std::move(transactionInfo) };

            for (const auto& r: m_replicators) {
                std::thread( [r] {
                    r->onApprovalTransactionHasBeenPublished(*TestEnvironment::m_approvalTransactionInfo);
                }).detach();
            }
        }
    }
