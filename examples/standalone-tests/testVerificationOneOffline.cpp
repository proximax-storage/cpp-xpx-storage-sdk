...
#define TEST_NAME VerificationOneOffline
...
#define ENVIRONMENT_CLASS JOIN(TEST_NAME, TestEnvironment)
    ...
    TEST(ModificationTest, TEST_NAME)
    {
        ...
        ENVIRONMENT_CLASS env(
                ...
                SANDBOX_ROOT_FOLDER, USE_TCP, 1, 1, 1024 * 1024, NUMBER_OF_REPLICATORS - 1);
        ...
        {
            for ( uint i = 0; i < opinion.m_opinions.size() - 1; i++ )
            {

                EXPECT_EQ(opinion.m_opinions[i], 1);
            }
            EXPECT_EQ(opinion.m_opinions.back(), 0);
        }
    }
