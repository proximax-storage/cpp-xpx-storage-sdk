...
static void     clientDownloadFsTree( const ReplicatorList& replicators, const InfoHash& );
static InfoHash clientModifyDrive( const ActionList&, const ReplicatorList& replicators );
static void     clientDownloadFiles( int fileNumber, Folder& folder, const ReplicatorList& replicators );
...
ReplicatorList replicatorsList;
...
int main() try {
    ...
    auto clientKeyPair = sirius::crypto::KeyPair::FromPrivate(sirius::crypto::PrivateKey::FromString( CLIENT_PRIVATE_KEY ));
    ...
    replicatorsList.emplace_back( ReplicatorInfo{ {ip, REPLICATOR_PORT},
...
//    LOG( "rootHash=" << toString(rootHash) );
    ...
    gClientSession->setDownloadChannel( replicatorsList, downloadChannelKey1 );

    client.openDownloadChannel(downloadChannelKey1.array(), 1024*1024, replicatorsList, { clientKeyPair.publicKey() });
...
//
static void clientDownloadFsTree( const ReplicatorList& replicators, const InfoHash& rootHash )
{
    LOG("clientDownloadFsTree()" << " " << replicators[0].address() << ":" << replicators[0].port() );
...
// clientModifyDrive
//
static InfoHash clientModifyDrive( const ActionList& actionList, const ReplicatorList& replicators )
{
    actionList.dbgPrint();
    ...
    InfoHash hash = gClientSession->addActionListToSession( actionList, replicators, modifyTransactionHash1, tmpFolder );
...
// Client: read files
//
static void clientDownloadFilesR( const Folder& folder, const ReplicatorList& replicators )
{
    for( const auto& child: folder.childs() )
    {
        if ( isFolder(child) )
        {
            clientDownloadFilesR( getFolder(child), replicators );
        }
        else
        {
...
static void clientDownloadFiles( int fileNumber, Folder& fsTree, const ReplicatorList& replicators )
{
    isDownloadCompleted = false;
    ...
    clientDownloadFilesR( fsTree, replicators );
