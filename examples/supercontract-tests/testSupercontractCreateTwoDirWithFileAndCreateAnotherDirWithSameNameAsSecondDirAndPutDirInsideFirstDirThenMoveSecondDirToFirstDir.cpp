...
#define TEST_NAME SupercontractCreateTwoDirWithFileAndCreateAnotherDirWithSameNameAsSecondDirAndPutDirInsideFirstDirThenMoveSecondDirToFirstDir
...
#define ENVIRONMENT_CLASS JOIN(TEST_NAME, TestEnvironment)
    ...
    namespace {
        ...
        public:
            ...
            TestCreateDirAndMove(ENVIRONMENT_CLASS& env)
                : m_env(env) {}
        ...
        public:
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
                ...
                for (auto const& [key, val] : files) {
                    if(isFile(val)){
						...
						ASSERT_TRUE(files2.size() == 1);
                        for (auto const& [key, val2] : files2) {
                            ASSERT_TRUE(isFile(val2));
                            const auto& file = getFile(val2);
                            ASSERT_TRUE(file.name() == "test2.txt");
                        }
					}
                }
			...
			void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests2", "tests/tests2", [this](auto res) { onFileMoved(res); } });
            }
        ...
        TEST(SupercontractTest, TEST_NAME) {
			...
			TestCreateDirAndMove handler2(env);
