...
#define TEST_NAME SupercontractCreateTwoDirWithFileThenSwapFilesPositionsInThreeStorageButFirstSandboxFALSE
...
#define ENVIRONMENT_CLASS JOIN(TEST_NAME, TestEnvironment)
    ...
    namespace {
        ...
        public:
            ...
            TestSwapThree(ENVIRONMENT_CLASS& env)
                : m_env(env) {}
        ...
        public:
			...
			void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
				ASSERT_TRUE(res);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests2/test2.txt", "tests/test2.txt", [this](auto res) { onFileMoved(res); } });
			}
        ...
        TEST(SupercontractTest, TEST_NAME) {
			...
			TestSwapThree handler4(env);
            handler4.m_driveKey = driveKey;
            env.initiateManualModifications(driveKey,
                InitiateModificationsRequest{ randomByteArray<Hash256>(), [&](auto res) { handler4.onInitiatedModifications(res); } });

            handler4.p.get_future().wait();
        }
    } // namespace
