...
#define TEST_NAME SupercontractMoveWhileIterator
...
class TestHandlerMoveWhileIterating {
public:
    std::promise<void> p;
    DriveKey m_driveKey;
    uint64_t m_fileId;
    ...
    TestHandlerMoveWhileIterating(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
public:
    ...
    void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
        ASSERT_TRUE(res);
        m_env.folderIteratorCreate(m_driveKey, FolderIteratorCreateRequest{"tests", true, [this](auto res) { onIteratorCreated(res); }});
    }
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    TestHandlerMoveWhileIterating handlerm(env);
