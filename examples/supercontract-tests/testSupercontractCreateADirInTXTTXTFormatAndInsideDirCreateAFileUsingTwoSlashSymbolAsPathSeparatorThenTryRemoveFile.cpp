...
#define TEST_NAME SupercontractCreateADirInTXTTXTFormatAndInsideDirCreateAFileUsingTwoSlashSymbolAsPathSeparatorThenTryRemoveFile
    ...
    namespace {
        ...
        public:
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				ASSERT_TRUE(folder.name() == "tests.txt.txt");
                ...
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests.txt.txt//test.txt", [this](auto res) {
			...
			void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "tests.txt.txt//test.txt"	, [this](auto res) { onFileOpened(res); } });
            }

            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "tests.txt.txt", [this](auto res) { onDirCreated(res); } });
            }
		...
		public:
			...
			void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
				ASSERT_TRUE(res);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"tests.txt.txt//test.txt", [this](auto res) {
															onFileRemoved(res);
														}});
			}
