...
#define TEST_NAME SupercontractMoveFolderIteratorChild
...
class TestHandlerMoveFolderIteratorChild {
public:
    std::promise<void> p;
    DriveKey m_driveKey;
    uint64_t m_fileId;
    ...
    TestHandlerMoveFolderIteratorChild(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    TestHandlerMoveFolderIteratorChild handlerm(env);
