...
#define TEST_NAME SupercontractRemoveEntriesWithExistingIterator
...
namespace {
    ...
    class TestHandlerRemoveEntriesWithExistingIterator {
    public:
        TestHandlerRemoveEntriesWithExistingIterator(
    ...
    public:
        ...
        void initiateSandboxModifications(
                ...
                const std::vector<std::pair<std::string, bool>>& testedEntries,
                bool recursiveIterator) {
            m_iteratorAbsolutePath = iteratorAbsolutePath;
            m_testedEntries = testedEntries;
            m_entryIter = m_testedEntries.begin();
            m_recursiveIterator = recursiveIterator;
    ...
    private:
        ...
        void onEntryRemovingAttempted(std::optional<RemoveFilesystemEntryResponse> response) {
            ASSERT_TRUE(response);

            // Attempts to remove
            // - a file or directory contained (directly or indirectly) in the directory with existing iterator; or
            // - a directory that contains (directly or indirectly) the directory with existing iterator
            // should fail:
            const bool expectedResult = m_entryIter->second;
            ASSERT_EQ(response->m_success, expectedResult);

            ++m_entryIter;
            if (m_entryIter != m_testedEntries.end()) {
                const auto& entryAbsolutePath = m_entryIter->first;
                m_environment.removeFsTreeEntry(
                        m_driveKey,
                        RemoveFilesystemEntryRequest{
                            entryAbsolutePath,
                            [this](auto response) { onEntryRemovingAttempted(response); }
                        });
            } else {
        ...
        void onIteratorCreated(std::optional<FolderIteratorCreateResponse> response) {
            ...
            // Attempt to remove a filesystem entry:
            const auto& entryAbsolutePath = m_entryIter->first;
            m_environment.removeFsTreeEntry(
                    m_driveKey,
                    RemoveFilesystemEntryRequest{
    ...
    private:
        ...
        std::vector<std::pair<std::string, bool>> m_testedEntries;
        std::vector<std::pair<std::string, bool>>::iterator m_entryIter;
        uint64_t m_folderIteratorId;
        bool m_recursiveIterator;
    };
    ...
    TEST(SupercontractTest, VARIANT_TEST_NAME(Recursive)) {
        ...
        TestHandlerRemoveEntriesWithExistingIterator mainHandler(environment, driveKey);
        mainHandler.initiateSandboxModifications(
                "outer/middle",
                {
                        {"outer/middle/test.txt", false},
                        {"outer/middle/inner", false},
                        {"outer", false},
                        {"outer/sibling/removable.txt", true},
                        {"outer/sibling", true},
                        {"removable", true}},
                true);
