...
#define TEST_NAME SupercontractRemoveWhileOpeningAnotherFile
...
class TestHandlerRemoveWhileOpeningAnotherFile {
...
public:
    ...
    TestHandlerRemoveWhileOpeningAnotherFile(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
public:
    ...
    void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
        ASSERT_TRUE(res);
        m_env.openFile(m_driveKey, OpenFileRequest{OpenFileMode::READ, "test2.txt", [this](auto res) { onFileOpened(res); }});
    }
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    TestHandlerRemoveWhileOpeningAnotherFile handler(env);
