...
#define TEST_NAME SupercontractMoveFolderIteratorParent
...
class TestHandlerMoveFolderIteratorParent {
public:
    std::promise<void> p;
    DriveKey m_driveKey;
    uint64_t m_fileId;
    ...
    TestHandlerMoveFolderIteratorParent(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    TestHandlerMoveFolderIteratorParent handlerm(env);
