...
#define TEST_NAME SupercontractCreateADirInCPPFormatAndInsideDirCreateAFileUsingTwoSlashSymbolAsPathSeparatorThenTryRemoveFile
    ...
    namespace {
        ...
        public:
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				ASSERT_TRUE(folder.name() == "tests.cpp");
                ...
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests.cpp//test.txt", [this](auto res) {
			...
			void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "tests.cpp//test.txt"	, [this](auto res) { onFileOpened(res); } });
            }

            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "tests.cpp", [this](auto res) { onDirCreated(res); } });
            }
		...
		public:
			...
			void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
				ASSERT_TRUE(res);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"tests.cpp//test.txt", [this](auto res) {
															onFileRemoved(res);
														}});
			}
