...
#define TEST_NAME SupercontractIsFileOnAFolder
...
public:
    ...
    void onDirectoryCreated(std::optional<CreateDirectoriesResponse> res) {
        ASSERT_TRUE(res);
        m_env.isFile(m_driveKey, PathIsFileRequest{"test", [this](auto res) {
                                                       onIsFile(res);
                                                   }});
    }
