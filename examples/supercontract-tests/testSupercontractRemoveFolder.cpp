...
#define TEST_NAME SupercontractRemoveFolder
...
public:
    ...
    void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
        ASSERT_TRUE(res);
        m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"test", [this](auto res) {
                                                                             onFileRemoved(res);
                                                                         }});
    }
...
public:
    ...
    void onReceivedFsTree(std::optional<FilesystemResponse> res) {
        ...
        m_env.getAbsolutePath(m_driveKey, AbsolutePathRequest{"test/test.txt", [this](auto res) {
    ...
    void onDirectoryCreated(std::optional<CreateDirectoriesResponse> res) {
        ASSERT_TRUE(res);
        m_env.openFile(m_driveKey, OpenFileRequest{OpenFileMode::WRITE, "test/test.txt", [this](auto res) { onFileOpened(res); }});
    }

    void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
        ASSERT_TRUE(res);
        m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{"test", [this](auto res) { onDirectoryCreated(res); }});
    }
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    CreateDummyFile handler_c(env);
    handler_c.m_driveKey = driveKey;
    env.initiateManualModifications(driveKey,
                                    InitiateModificationsRequest{randomByteArray<Hash256>(), [&](auto res) { handler_c.onInitiatedModifications(res); }});

    handler_c.p.get_future().wait();
