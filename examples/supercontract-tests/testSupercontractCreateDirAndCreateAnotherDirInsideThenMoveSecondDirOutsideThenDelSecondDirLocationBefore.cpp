...
#define TEST_NAME SupercontractCreateDirAndCreateAnotherDirInsideThenMoveSecondDirOutsideThenDelSecondDirLocationBefore
    ...
    namespace {
        ...
        public:
				...
				for (auto const& [key, val] : files) {
					if(isFile(val)){
						...
						for (auto const& [key, val] : files2) {
							...
							ASSERT_TRUE(file2.name() == "test2.txt");
						}
					}
				}
            ...
            void onDirCreated2(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "tests/tests2/test2.txt", [this](auto res) { onFileOpened2(res); } });
            }

            void onFileClosed(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "tests/tests2", [this](auto res) { onDirCreated2(res); } });
            }
        ...
        public:
			...
			void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
				ASSERT_TRUE(res);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests/tests2", "tests2", [this](auto res) { onFileMoved(res); } });
			}
        ...
        public:
			...
			void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
				ASSERT_TRUE(res);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"tests/tests2", [this](auto res) {
															onFileRemoved(res);
														}});
			}
