...
#define TEST_NAME SupercontractCreateThreeFilesWithNonEnglishContentThenCreateADirAndMoveThreeFilesToDir
    ...
    namespace {
        ...
        public:
			...
			void onReceivedAbsolutePath2(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "테스트", [this](auto res) {
            ...
            void onReceivedAbsolutePath(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "テスト", [this](auto res) {
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
                ...
                for (auto const& [_, child] : fsTree.childs()) {
					...
					if (file.name() == "测试") {
                        ASSERT_TRUE(file.name() == "测试");
                    }
					else if (file.name() == "テスト"){
                        ASSERT_TRUE(file.name() == "テスト");
                    }
                    else {
                        ASSERT_TRUE(file.name() == "테스트");
                    }
                }
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "测试", [this](auto res) {
			...
			void onFileClosed2(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "테스트", [this](auto res) { onFileOpened3(res); } });
            }
            ...
            void onFileClosed(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "テスト", [this](auto res) { onFileOpened2(res); } });
            }
            ...
            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "测试", [this](auto res) { onFileOpened(res); } });
            }
		...
		public:
			...
			void onReceivedAbsolutePath2(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests/테스트", [this](auto res) {
                                                                          onReceivedAbsolutePath3(res);
                                                                      } });
            }
            ...
            void onReceivedAbsolutePath(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests/テスト", [this](auto res) {
                                                                          onReceivedAbsolutePath2(res);
                                                                      } });
            }
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
                ...
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests/测试", [this](auto res) {
                                                                          onReceivedAbsolutePath(res);
                                                                      } });
            }
			...
			void onFileMoved3(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.applySandboxManualModifications(m_driveKey, ApplySandboxModificationsRequest{ true, [this](auto res) {
														   onAppliedSandboxModifications(res);
			...
			void onFileMoved2(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "테스트", "tests/테스트", [this](auto res) { onFileMoved3(res); } });
            }			
			
			void onFileMoved(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "テスト", "tests/テスト", [this](auto res) { onFileMoved2(res); } });
            }
			
			void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "测试", "tests/测试", [this](auto res) { onFileMoved(res); } });
            }
