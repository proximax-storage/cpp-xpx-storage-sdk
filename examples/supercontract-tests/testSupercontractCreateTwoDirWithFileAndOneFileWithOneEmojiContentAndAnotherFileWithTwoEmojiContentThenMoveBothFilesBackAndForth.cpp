...
#define TEST_NAME SupercontractCreateTwoDirWithFileAndOneFileWithOneEmojiContentAndAnotherFileWithTwoEmojiContentThenMoveBothFilesBackAndForth
...
#define ENVIRONMENT_CLASS JOIN(TEST_NAME, TestEnvironment)
    ...
    namespace {
        ...
        public:
			...
			void onFileMoved4(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.applySandboxManualModifications(m_driveKey, ApplySandboxModificationsRequest{ true, [this](auto res) {
																		   onAppliedSandboxModifications(res);
			...
			void onFileMoved3(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests/test2.txt", "tests2/test2.txt", [this](auto res) { onFileMoved4(res); } });
            }

			void onFileMoved2(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests2/test2.txt", "tests/test2.txt", [this](auto res) { onFileMoved3(res); } });
            }

			void onFileMoved(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests2/test.txt", "tests/test.txt", [this](auto res) { onFileMoved2(res); } });
            }
