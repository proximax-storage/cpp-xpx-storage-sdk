...
#define TEST_NAME SupercontractCreateADirAndCreateAFileOutsideDirAndBothDirAndFileUseSameNameThenCreateAnotherDirAndMoveNameOfDirAndFile
...
#define ENVIRONMENT_CLASS JOIN(TEST_NAME, TestEnvironment)
    ...
    namespace {
        ...
        public:
            ...
            TestCreateDirAndFile(ENVIRONMENT_CLASS& env)
                : m_env(env) {}
        ...
        public:
            ...
            void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "tests", [this](auto res) { onFileOpened(res); } });
            }
		...
		public:
			void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				for (auto const& [key, val] : files) {
					...
					ASSERT_TRUE(folder2.name() == "tests");
                    ASSERT_TRUE(folder2.childs().size() == 0);
				}
                p.set_value();
            }
			...
			void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests", "tests2/tests", [this](auto res) { onFileMoved(res); } });
            }
        ...
        TEST(SupercontractTest, TEST_NAME) {
            ...
            TestCreateDirAndFile handler(env);
