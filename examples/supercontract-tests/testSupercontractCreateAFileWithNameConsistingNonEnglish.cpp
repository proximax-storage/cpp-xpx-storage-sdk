...
#define TEST_NAME SupercontractCreateAFileWithNameConsistingNonEnglish
    ...
    namespace {
        ...
        public:
			...
			void onReceivedAbsolutePath2(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "테스트.txt", [this](auto res) {
            ...
            void onReceivedAbsolutePath(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "テスト.txt", [this](auto res) {
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				for (auto const& [_, child] : fsTree.childs()) {
                    ...
                    if (file.name() == "测试.txt") {
                        ASSERT_TRUE(file.name() == "测试.txt");
                    }
                    else if (file.name() == "テスト.txt"){
                        ASSERT_TRUE(file.name() == "テスト.txt");
                    }
					else if (file.name() == "테스트.txt"){
						ASSERT_TRUE(file.name() == "테스트.txt");
					}
                }
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "测试.txt", [this](auto res) {
			...
			void onFileClosed2(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "테스트.txt",
					[this](auto res) { onFileOpened3(res); } });
            }
            ...
            void onFileClosed(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "テスト.txt",
					[this](auto res) { onFileOpened2(res); } });
            }
            ...
            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "测试.txt",
