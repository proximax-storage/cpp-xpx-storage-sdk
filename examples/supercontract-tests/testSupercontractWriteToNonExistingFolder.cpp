...
#define TEST_NAME SupercontractWriteToNonExistingFolder
...
public:
    ...
    WriteNoneExisting(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
public:
    ...
    void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
        ASSERT_TRUE(res);
        m_env.openFile(m_driveKey, OpenFileRequest{OpenFileMode::WRITE, "uuh/test.txt", [this](auto res) { onFileOpened(res); }});
    }
