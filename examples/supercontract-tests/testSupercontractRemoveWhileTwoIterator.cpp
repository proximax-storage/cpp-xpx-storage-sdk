...
#define TEST_NAME SupercontractRemoveWhileTwoIterator
...
class TestHandlerRemoveWhileTwoIterator {
...
public:
    ...
    void onIteratorDestroyed(std::optional<FolderIteratorDestroyResponse> res) {
        ASSERT_TRUE(res);
        ASSERT_TRUE(res->success);
        m_env.folderIteratorDestroy(m_driveKey, FolderIteratorDestroyRequest{m_fileId2, [this](auto res) {
                                                                                 onIteratorDestroyed2(res);
                                                                             }});
    }
    ...
    void onIterCreated2(std::optional<FolderIteratorCreateResponse> res) {
        ASSERT_TRUE(res);
    ...
    void onIterCreated(std::optional<FolderIteratorCreateResponse> res) {
        ...
        ASSERT_TRUE(res->m_id);
        m_fileId = *res->m_id;
        m_env.folderIteratorCreate(m_driveKey, FolderIteratorCreateRequest{"tests", true, [this](auto res) { onIterCreated2(res); }});
    }
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    TestHandlerRemoveWhileTwoIterator handler_r(env);
