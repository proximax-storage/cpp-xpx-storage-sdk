...
#define TEST_NAME SupercontractCreateThreeFilesWithNonEnglishContentThenTryRemoveThreeFiles
    ...
    namespace {
		...
		public:
			...
			void onFileRemoved2(std::optional<RemoveFilesystemEntryResponse> res) {
				ASSERT_TRUE(res);
				ASSERT_TRUE(res->m_success);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"테스트", [this](auto res) {
															onFileRemoved3(res);
														}});
			}
			...
			void onFileRemoved(std::optional<RemoveFilesystemEntryResponse> res) {
				ASSERT_TRUE(res);
				ASSERT_TRUE(res->m_success);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"テスト", [this](auto res) {
															onFileRemoved2(res);
														}});
			}
			...
			void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
				ASSERT_TRUE(res);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"测试", [this](auto res) {
															onFileRemoved(res);
														}});
			}
