...
#define TEST_NAME SupercontractCreateADirWithTXTFormatThenCreateAnotherDirAndMoveFirstDirToSecondDir
    ...
    namespace {
        ...
        public:
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				ASSERT_TRUE(folder.name() == "tests.txt");
                ...
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests.txt/test.txt", [this](auto res) {
			...
			void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "tests.txt/test.txt"	, [this](auto res) { onFileOpened(res); } });
            }

            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "tests.txt", [this](auto res) { onDirCreated(res); } });
            }
		...
		public:
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				for (auto const& [key, val] : files) {
					...
					ASSERT_TRUE(folder2.name() == "tests.txt");
					const auto& files2 = folder2.childs();
					for (auto const& [key, val] : files2) {
						ASSERT_TRUE(isFile(val));
						const auto& file2 = getFile(val);
						ASSERT_TRUE(file2.name() == "test.txt");
					}
				}
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests2/tests.txt/test.txt", [this](auto res) {
                                                                          onReceivedAbsolutePath(res);
                                                                      } });
            }
			...
			void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests.txt", "tests2/tests.txt", [this](auto res) { onFileMoved(res); } });
            }

            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "tests2", [this](auto res) { onDirCreated(res); } });
            }
