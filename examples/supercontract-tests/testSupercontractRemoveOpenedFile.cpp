...
#define TEST_NAME SupercontractRemoveOpenedFile
...
namespace {
    ...
    class TestHandlerRemoveOpenedFile {
    public:
        TestHandlerRemoveOpenedFile(
    ...
    private:
        ...
        void onFileRemovingAttempted(std::optional<RemoveFilesystemEntryResponse> response) {
            ASSERT_TRUE(response);

            // Attempts to remove an opened file should fail:
            ASSERT_FALSE(response->m_success);
        ...
        void onFileOpened(std::optional<OpenFileResponse> response) {
            ...
            m_environment.removeFsTreeEntry(
                    ...
                    RemoveFilesystemEntryRequest{
                        m_fileAbsolutePath,
                        [this](auto response) { onFileRemovingAttempted(response); }
                    });
        }
    ...
    TEST(SupercontractTest, TEST_NAME) {
        ...
        TestHandlerRemoveOpenedFile mainHandler(environment, driveKey);
