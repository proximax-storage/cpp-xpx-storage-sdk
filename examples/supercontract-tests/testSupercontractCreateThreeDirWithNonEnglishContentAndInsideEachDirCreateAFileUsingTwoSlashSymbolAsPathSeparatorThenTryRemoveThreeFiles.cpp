...
#define TEST_NAME SupercontractCreateThreeDirWithNonEnglishContentAndInsideEachDirCreateAFileUsingTwoSlashSymbolAsPathSeparatorThenTryRemoveThreeFiles
    ...
    namespace {
        ...
        public:
			...
			void onReceivedAbsolutePath2(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "테스트//test.txt", [this](auto res) {
            ...
            void onReceivedAbsolutePath(std::optional<FileInfoResponse> res) {
                ...
                auto content = stream.str();
                ASSERT_EQ(content, "data");
				m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "テスト//test.txt", [this](auto res) {
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
                ...
                for (auto const& [_, child] : fsTree.childs()) {
                    ...
                    else if (folder.name() == "テスト") {
                        ASSERT_TRUE(folder.name() == "テスト");
                        const auto& files = folder.childs();
                        for (auto const& [key, val] : files) {
                            ASSERT_TRUE(isFile(val));
                            const auto& file = getFile(val);
                ...
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "测试//test.txt", [this](auto res) {
			...
			void onDirCreated3(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "테스트//test.txt", [this](auto res) { onFileOpened3(res); } });
            }

			void onFileClosed2(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "테스트", [this](auto res) { onDirCreated3(res); } });
            }
			...
			void onDirCreated2(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "テスト//test.txt", [this](auto res) { onFileOpened2(res); } });
            }
			
            void onFileClosed(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "テスト", [this](auto res) { onDirCreated2(res); } });
            }
            ...
            void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "测试//test.txt"	, [this](auto res) { onFileOpened(res); } });
            }

            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "测试", [this](auto res) { onDirCreated(res); } });
            }
		...
		public:
			...
			void onFileRemoved3(std::optional<RemoveFilesystemEntryResponse> res) {
				ASSERT_TRUE(res);
				ASSERT_TRUE(res->m_success);
				m_env.applySandboxManualModifications(m_driveKey, ApplySandboxModificationsRequest{ true, [this](auto res) {
														   onAppliedSandboxModifications(res);
			...
			void onFileRemoved2(std::optional<RemoveFilesystemEntryResponse> res) {
				ASSERT_TRUE(res);
				ASSERT_TRUE(res->m_success);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"테스트//test.txt", [this](auto res) {
															onFileRemoved3(res);
														}});
			}
			...
			void onFileRemoved(std::optional<RemoveFilesystemEntryResponse> res) {
				ASSERT_TRUE(res);
				ASSERT_TRUE(res->m_success);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"テスト//test.txt", [this](auto res) {
															onFileRemoved2(res);
														}});
			}
			...
			void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
				ASSERT_TRUE(res);
				m_env.removeFsTreeEntry(m_driveKey, RemoveFilesystemEntryRequest{"测试//test.txt", [this](auto res) {
															onFileRemoved(res);
														}});
			}
