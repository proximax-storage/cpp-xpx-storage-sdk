...
#define TEST_NAME SupercontractMoveDiscardMiddleStorageModification
    ...
    namespace {
        ...
        public:
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
                ...
                std::vector<std::string> expected = {"test.txt"};
            ...
            void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{"test.txt", "moved/test.txt", [this](auto res) { onFileMoved(res); }});
            }
