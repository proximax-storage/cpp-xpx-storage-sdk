...
#define TEST_NAME SupercontractCreateADirAndCreateAnotherDirWithNameSymbolSlashOnlyInsideFirstDirAndCreateAFileWIthinSecondDir
    ...
    namespace {
        ...
        public:
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				for (auto const& [_, child] : files) {
					if(isFile(child)){
						...
						ASSERT_TRUE(isFolder(child));
						const auto& folder2 = getFolder(child);
						ASSERT_TRUE(folder2.name() == "");
						const auto& files2 = folder2.childs();
						ASSERT_TRUE(files2.size() == 0);
					}
                }
            ...
            void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "tests//", [this](auto res) { onDirCreated2(res); } });
            }
