...
#define TEST_NAME SupercontractMoveAndReadBeforeModification
...
class TestHandlerReadMovedFile {
public:
    std::promise<void> p;
    DriveKey m_driveKey;
    uint64_t m_fileId;
    ...
    TestHandlerReadMovedFile(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    TestHandlerReadMovedFile handlerm(env);
