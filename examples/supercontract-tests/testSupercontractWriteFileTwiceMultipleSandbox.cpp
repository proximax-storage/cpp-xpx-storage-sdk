...
#define TEST_NAME SupercontractWriteFileTwiceMultipleSandbox
...
class WriteFileTwiceMultipleSandbox {
...
public:
    ...
    WriteFileTwiceMultipleSandbox(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
public:
    void onReceivedAbsolutePath(std::optional<FileInfoResponse> res) {
        ...
        ASSERT_EQ(content, "data data data data");
        p.set_value();
    }
    ...
    void onFileOpened2(std::optional<OpenFileResponse> res) {
        ...
        std::string buffer = "data data data data";
    ...
    void onSandboxModificationsInitiated2(std::optional<InitiateSandboxModificationsResponse> res) {
        ASSERT_TRUE(res);
        m_env.openFile(m_driveKey, OpenFileRequest{OpenFileMode::WRITE, "test.txt", [this](auto res) { onFileOpened2(res); }});
    }
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    WriteFileTwiceMultipleSandbox handler(env);
