...
#define TEST_NAME SupercontractCreateADirWithNameConsistingNonEnglish
    ...
    namespace {
        ...
        public:
			...
			void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				for (auto const& [_, child] : fsTree.childs()) {
                    ...
                    if (folder.name() == "测试") {
                        ...
                        const auto& files2 = folder.childs();
					...
					else if(folder.name() == "테스트"){
                        ASSERT_TRUE(folder.name() == "테스트");
                        const auto& files3 = folder.childs();
                        for (auto const& [key, val] : files3) {
                            ASSERT_TRUE(isFile(val));
                            const auto& file = getFile(val);
                            ASSERT_TRUE(file.name() == "test.txt");
                        }
			...
			void onFileClosed2(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "테스트", 
					[this](auto res) { onDirCreated3(res); } });
            }
            ...
            void onFileClosed(std::optional<CloseFileResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "テスト", 
					[this](auto res) { onDirCreated2(res); } });
            }
            ...
            void onSandboxModificationsInitiated(std::optional<InitiateSandboxModificationsResponse> res) {
                ASSERT_TRUE(res);
                m_env.createDirectories(m_driveKey, CreateDirectoriesRequest{ "测试", 
