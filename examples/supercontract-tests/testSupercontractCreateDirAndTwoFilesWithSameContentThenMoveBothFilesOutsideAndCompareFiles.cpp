...
#define TEST_NAME SupercontractCreateDirAndTwoFilesWithSameContentThenMoveBothFilesOutsideAndCompareFiles
    ...
    namespace {
        ...
        public:
			...
			void onFileMoved(std::optional<MoveFilesystemEntryResponse> res) {
                ASSERT_TRUE(res);
                ASSERT_TRUE(res->m_success);
				m_env.moveFsTreeEntry(m_driveKey, MoveFilesystemEntryRequest{ "tests/test.txt", "test.txt", [this](auto res) { onFileMoved2(res); } });
            }
