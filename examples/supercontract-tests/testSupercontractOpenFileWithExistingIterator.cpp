...
#define TEST_NAME SupercontractOpenFileWithExistingIterator
...
namespace {
    ...
    class TestHandlerOpenFileWithExistingIterator {
    public:
        TestHandlerOpenFileWithExistingIterator(
    ...
    public:
        ...
        void initiateSandboxModifications(
                ...
                const std::vector<std::pair<std::string, bool>>& testedFiles,
                OpenFileMode openFileMode,
                bool recursiveIterator) {
            m_iteratorAbsolutePath = iteratorAbsolutePath;
            m_testedFiles = testedFiles;
            m_fileIter = m_testedFiles.begin();
    ...
    private:
        ...
        void processNextFile() {
            ++m_fileIter;
            if (m_fileIter != m_testedFiles.end()) {
                ...
                m_environment.openFile(
                        ...
                        OpenFileRequest{
                            m_openFileMode,
                            fileAbsolutePath,
                            [this](auto response) { onFileOpeningAttempted(response); }
                        });
            } else {
        ...
        void onFileOpeningAttempted(std::optional<OpenFileResponse> response) {
            ASSERT_TRUE(response);

            // On attempt to open a file that is contained (directly or indirectly)
            // in a directory with existing iterator, response should not return a file ID:
            const bool expectedResult = m_fileIter->second;
            ASSERT_EQ(static_cast<bool>(response->m_fileId), expectedResult);

            if (response->m_fileId) {
                // Close the file before testing other files:
    ...
    private:
        ...
        std::vector<std::pair<std::string, bool>> m_testedFiles;
        std::vector<std::pair<std::string, bool>>::iterator m_fileIter;
    ...
    TEST(SupercontractTest, VARIANT_TEST_NAME(Recursive)) {
        ...
        TestHandlerOpenFileWithExistingIterator mainHandler(environment, driveKey);
        mainHandler.initiateSandboxModifications(
                "outer/middle",
                {
                        {"outer/middle/test.txt", false},
                        {"outer/middle/inner/test.txt", false},
                        {"outer/openable.txt", true},
                        {"openable.txt", true}},
                OpenFileMode::READ,
                true);
