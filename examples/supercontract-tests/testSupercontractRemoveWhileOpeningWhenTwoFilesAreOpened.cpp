...
#define TEST_NAME SupercontractRemoveWhenTwoFilesAreOpened
...
class TestHandlerRemoveWhenTwoFilesAreOpened {
...
public:
    ...
    TestHandlerRemoveWhenTwoFilesAreOpened(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
public:
    ...
    void onFileClosed(std::optional<CloseFileResponse> res) {
        ASSERT_TRUE(res);
        ASSERT_TRUE(res->m_success);
        m_env.closeFile(m_driveKey, CloseFileRequest{m_fileId2, [this](auto res) {
                                                         onFileClosed2(res);
                                                     }});
    }
    ...
    void onFileOpened(std::optional<OpenFileResponse> res) {
        ...
        m_env.openFile(m_driveKey, OpenFileRequest{OpenFileMode::READ, "test2.txt", [this](auto res) { onFileOpened2(res); }});
    }
...
public:
    ...
    CreateDummy2(ENVIRONMENT_CLASS& env)
        : m_env(env) {}
...
TEST(SupercontractTest, TEST_NAME) {
    ...
    CreateDummy2 handler_d(env);
    handler_d.m_driveKey = driveKey;
    env.initiateManualModifications(driveKey,
                                    InitiateModificationsRequest{randomByteArray<Hash256>(), [&](auto res) { handler_d.onInitiatedModifications(res); }});

    handler_d.p.get_future().wait();

    TestHandlerRemoveWhenTwoFilesAreOpened handler(env);
