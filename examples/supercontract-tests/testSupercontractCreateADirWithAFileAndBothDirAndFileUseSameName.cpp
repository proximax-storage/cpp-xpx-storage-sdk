...
#define TEST_NAME SupercontractCreateADirWithAFileAndBothDirAndFileUseSameName
        ...
        public:
            ...
            void onReceivedFsTree(std::optional<FilesystemResponse> res) {
				...
				for (auto const& [key, val] : files) {
					...
					ASSERT_TRUE(file.name() == "tests");
				}
                m_env.getAbsolutePath(m_driveKey, FileInfoRequest{ "tests/tests", [this](auto res) {
            ...
            void onDirCreated(std::optional<CreateDirectoriesResponse> res) {
                ASSERT_TRUE(res);
                m_env.openFile(m_driveKey, OpenFileRequest{ OpenFileMode::WRITE, "tests/tests", [this](auto res) { onFileOpened(res); } });
            }
