...
protected:
    ...
    explicit ContentDriveTask(
            const DriveTaskType& type,
            TaskController& drive,
            ModifyOpinionController& opinionTaskController,
            const FlatDrivePaths& flatDrivePaths)
            : BaseDriveTask(type, drive, flatDrivePaths), m_opinionController(opinionTaskController),
              m_sandboxCalculated(false), m_stopped(false)
    {}

    virtual void myRootHashIsCalculated( const uint64_t& sandboxDriveSize,
                                         ...
                                         const uint64_t& sandboxMetafilesSize )
    {
        ...
        // Notify
        if ( m_drive.getDbgReplicatorEventHandler() )
        {
            m_drive.getDbgReplicatorEventHandler()->rootHashIsCalculated(
                    m_drive.getReplicator(), 
                    m_drive.getDriveKey(), 
        ...
        m_opinionController.createMyOpinion( m_sandboxRootHash, sandboxDriveSize, m_sandboxFsTree, sandboxMetafilesSize,  );
    }
    ...
    virtual void modifyIsCompleted()
    {
        ...
        m_drive.getFsTree() = m_sandboxFsTree;
        m_drive.getRootHash() = *m_sandboxRootHash;
        m_drive.getFsTreeHandle() = std::move(*m_sandboxFsTreeLtHandle);
...
//        m_isSynchronizing = false;
    ...
    void sendSingleApprovalTransaction()
    {
        ...
        m_drive.getReplicatorEventHandler()->singleModifyApprovalTransactionIsReady(m_drive.getReplicator(), std::move(copy));
    }
    ...
    void getSandboxDriveSizes( uint64_t& metaFilesSize, uint64_t& driveSize ) const
    {
        ...
        if ( fs::exists(m_flatDrivePaths.m_sandboxRootPath,err) )
        {
            metaFilesSize = fs::file_size( m_flatDrivePaths.m_sandboxFsTreeTorrent);
            driveSize = 0;
            m_sandboxFsTree->getSizes( m_flatDrivePaths.m_driveFolder, m_flatDrivePaths.m_torrentFolder, metaFilesSize, driveSize );
    ...
    uint64_t sandboxFsTreeSize() const
    {
        ...
        if ( fs::exists(m_flatDrivePaths.m_sandboxFsTreeFile,err) )
        {
            return fs::file_size( m_flatDrivePaths.m_sandboxFsTreeFile, err );
        }
        return 0;
    }
...
private:
    ...
    ModificationRequest m_modificationRequest;
...
public:
    ...
    explicit ModificationRequestDriveTask(
            const ModificationRequest& request,
            TaskController& drive,
            ModifyOpinionController& opinionTaskController,
            const FlatDrivePaths& flatDrivePaths)
            : ContentDriveTask(DriveTaskType::MODIFICATION_REQUEST, drive, opinionTaskController, flatDrivePaths)
            , m_modificationRequest(request)
    ...
    void run() override
    {
        ...
        m_opinionController.increaseExpectedCumulativeDownload( m_modificationRequest.m_maxDataSize );

        auto& opinionTrafficIdentifier = m_opinionController.getOpinionTrafficIdentifier();
        _ASSERT(!opinionTrafficIdentifier);

        _LOG("started modification")

        opinionTrafficIdentifier = m_modificationRequest.m_transactionHash.array();
        ...
        if (auto session = m_drive.getSession().lock(); session)
        {
            ...
            m_downloadingLtHandle = session->download(DownloadContext(
                                                              ...
                                                              m_modificationRequest.m_clientDataInfoHash,
                                                              *opinionTrafficIdentifier,
                                                              0, //todo
                                                              ""),
                                                      m_flatDrivePaths.m_sandboxRootPath,
                                                      m_drive.getUploaderKeys());
        }
    }

    bool processedOpinion (
            const ApprovalTransactionInfo& anOpinion ) override
    {
        // In this case Replicator is able to verify all data in the opinion
        if ( m_myOpinion &&
             m_modificationRequest.m_transactionHash.array() == anOpinion.m_modifyTransactionHash &&
             validateOpinion(anOpinion) )
        {
            auto& otherOpinions = m_opinionController.getOtherOpinions( anOpinion.m_modifyTransactionHash );
            otherOpinions[anOpinion.m_opinions[0].m_replicatorKey] = anOpinion;
        }
        return true;
    }
            ...
            const PublishedModificationApprovalTransactionInfo& transaction) override
    {
        ...
        if ( m_modificationRequest.m_transactionHash == transaction.m_modifyTransactionHash
             && m_modifyUserDataReceived )
        {
            if ( m_sandboxCalculated )
            {
                const auto& v = transaction.m_replicatorKeys;
                auto it = std::find( v.begin(), v.end(), m_drive.getReplicator().replicatorKey().array());
...
private:
                         ...
                         const std::string& errorText)
    {
        ...
        _ASSERT(m_modificationRequest.m_clientDataInfoHash == infoHash)

        if (code == download_status::failed)
        {
            if (m_drive.getDbgReplicatorEventHandler())
            {
                m_drive.getDbgReplicatorEventHandler()->modifyTransactionEndedWithError(m_drive.getReplicator(), m_drive.getDriveKey(), m_modificationRequest,
                                                                   errorText, 0);
            }
            modifyIsCompleted();
            return;
    ...
    void modifyDriveInSandbox()
    {
        ...
        if (!fs::exists(m_flatDrivePaths.m_clientDataFolder, err) || !fs::is_directory(m_flatDrivePaths.m_clientDataFolder, err))
        {
            _LOG_ERR("modifyDriveInSandbox: 'client-data' is absent; m_clientDataFolder=" << m_flatDrivePaths.m_clientDataFolder);
            if (m_drive.getDbgReplicatorEventHandler())
            {
                m_drive.getDbgReplicatorEventHandler()->modifyTransactionEndedWithError(m_drive.getReplicator(), m_drive.getDriveKey(), m_modificationRequest,
                                                                   "modify drive: 'client-data' is absent", -1);
            }
        ...
        if (!fs::exists(m_flatDrivePaths.m_clientActionListFile, err))
        {
            _LOG_ERR("modifyDriveInSandbox: 'ActionList.bin' is absent: "
                             << m_flatDrivePaths.m_clientActionListFile);
            if (m_drive.getDbgReplicatorEventHandler())
            {
                m_drive.getDbgReplicatorEventHandler()->modifyTransactionEndedWithError(m_drive.getReplicator(), m_drive.getDriveKey(), m_modificationRequest,
                                                                   "modify drive: 'ActionList.bin' is absent", -1);
            }
            m_drive.enqueueMainThreadTask([=, this]()
               {
                   modifyIsCompleted();
        ...
        actionList.deserialize(m_flatDrivePaths.m_clientActionListFile);

        // Make copy of current FsTree
        m_sandboxFsTree->deserialize(m_flatDrivePaths.m_fsTreeFile);
        ...
        for (const Action& action : actionList)
        {
            ...
            switch (action.m_actionId)
            {
                ...
                case action_list_id::upload:
                {
                    ...
                    fs::path clientFile = m_flatDrivePaths.m_clientDriveFolder / action.m_param2;
                    ...
                    try
                    {
                        ...
                        InfoHash fileHash = calculateInfoHashAndCreateTorrentFile(clientFile, m_drive.getDriveKey(),
                                                                                  m_flatDrivePaths.m_torrentFolder,
                                                                                  "");
                        ...
                        std::string newFileName = m_flatDrivePaths.m_driveFolder / hashToFileName(fileHash);
        ...
        m_sandboxFsTree->doSerialize(m_flatDrivePaths.m_sandboxFsTreeFile);

        m_sandboxRootHash = createTorrentFile(m_flatDrivePaths.m_sandboxFsTreeFile,
                                                 m_drive.getDriveKey(),
                                                 m_flatDrivePaths.m_sandboxRootPath,
                                                 m_flatDrivePaths.m_sandboxFsTreeTorrent);

        uint64_t metaFilesSize;
        uint64_t sandboxDriveSize;
        getSandboxDriveSizes(metaFilesSize, sandboxDriveSize);
        uint64_t fsTreeSize = sandboxFsTreeSize();

        m_drive.enqueueMainThreadTask([=, this]() mutable
                                      {
                                          myRootHashIsCalculated(sandboxDriveSize,
                                                                 fsTreeSize,
                                                                 metaFilesSize);
                                      });
    }
...
protected:
    ...
    void modifyIsCompleted() override
    {
        ...
        m_drive.getDbgReplicatorEventHandler()->driveModificationIsCompleted(m_drive.getReplicator(), m_drive.getDriveKey(),
                                                                             m_modificationRequest.m_transactionHash,
                                                                             *m_sandboxRootHash);
...
private:
    ...
    void myOpinionIsCreated() override
    {
        ...
        } else
        {
            ...
            auto& transactionOpinions = m_opinionController.getOtherOpinions(m_modificationRequest.m_transactionHash);
    ...
    void shareMyOpinion()
    {
        ...
        {
            m_drive.getReplicator().sendMessage("opinion", replicatorIt.array(), os.str());
        }
    }
...
// check opinion number
        if ( m_myOpinion &&
             m_opinionController.getOtherOpinions( m_modificationRequest.m_transactionHash ).size() >= ((replicatorNumber) * 2) / 3 &&
            ...
            if (!m_modifyOpinionTimer)
            {
                if (auto session = m_drive.getSession().lock(); session)
                {
                    m_modifyOpinionTimer = session->startTimer(
                            m_drive.getReplicator().getModifyApprovalTransactionTimerDelay(),
                            [this]()
                            { opinionTimerExpired(); });
                }
            }
    ...
    void opinionTimerExpired()
    {
        ...
        ApprovalTransactionInfo info = {    m_drive.getDriveKey().array(),
        ...
        const auto& opinions = m_opinionController.getOtherOpinions( m_modificationRequest.m_transactionHash );
        info.m_opinions.reserve( opinions.size()+1 );
        info.m_opinions.emplace_back(  m_myOpinion->m_opinions[0] );
        for( const auto& otherOpinion : opinions ) {
            info.m_opinions.emplace_back( otherOpinion.second.m_opinions[0] );
        }

        // notify event handler
        m_drive.getReplicatorEventHandler()->modifyApprovalTransactionIsReady( m_drive.getReplicator(), std::move(info) );
    ...
    void continueSynchronizingDriveWithSandbox() override
    {
        ...
        try
        {
            ...
            fs::rename( m_flatDrivePaths.m_sandboxFsTreeFile, m_flatDrivePaths.m_fsTreeFile );
            fs::rename( m_flatDrivePaths.m_sandboxFsTreeTorrent, m_flatDrivePaths.m_fsTreeTorrent );
            ...
            std::erase_if( torrentHandleMap, [] (const auto& it) { return !it.second.m_isUsed; } );
            ...
            for( auto& it : torrentHandleMap )
            {
                ...
                if ( !it.second.m_ltHandle.is_valid() )
                {
                    if ( auto session = m_drive.getSession().lock(); session )
                    {
                        ...
                        it.second.m_ltHandle = session->addTorrentFileToSession( m_flatDrivePaths.m_torrentFolder / fileName,
                                                                                 m_flatDrivePaths.m_driveFolder,
                                                                                 lt::sf_is_replicator );
                    }
                }
            }
            ...
            if ( auto session = m_drive.getSession().lock(); session )
            {
                m_sandboxFsTreeLtHandle = session->addTorrentFileToSession( m_flatDrivePaths.m_fsTreeTorrent,
                                                                   m_flatDrivePaths.m_fsTreeTorrent.parent_path(),
                                                                   lt::sf_is_replicator );
            }
...
private:
    ...
    CatchingUpRequest                m_request;
    std::deque<ModificationRequest>& m_modificationRequests;
...
public:
    void run() override
    {
        ...
        auto it = std::find_if( modificationRequests.begin(), modificationRequests.end(), [&]( const auto& item )
            {
                return item.m_transactionHash ==
                       m_request.m_modifyTransactionHash;
            } );
        if (it != modificationRequests.end() )
        {
            auto& opinionTrafficIdentifier = m_opinionController.getOpinionTrafficIdentifier();
            while (!m_modificationRequests.empty() and it != m_modificationRequests.begin() )
            {
                if ( !opinionTrafficIdentifier
                || *opinionTrafficIdentifier != m_modificationRequests.front().m_transactionHash.array() )
                {
                    m_replicator.removeModifyDriveInfo(m_modificationRequests.front().m_transactionHash.array());
                }
                m_opinionController.increaseExpectedCumulativeDownload(m_modificationRequests.front().m_maxDataSize);
                m_modificationRequests.pop_front();
            }
            m_opinionController.increaseExpectedCumulativeDownload(m_modificationRequests.front().m_maxDataSize);
            m_modificationRequests.pop_front();
            if ( opinionTrafficIdentifier &&
...
private:
    ...
    void continueSynchronizingDriveWithSandbox() override
    {
        ...
        try
        {
            ...
            for( const auto& fileHash : m_catchingUpFileSet )
            {
                ...
                if ( auto session = m_drive.getSession().lock(); session )
                {
                    auto tHandle = session->addTorrentFileToSession( m_flatDrivePaths.m_torrentFolder / fileName,
                                                                     m_flatDrivePaths.m_driveFolder,
                                                                     lt::sf_is_replicator );
    ...
    void startDownloadMissingFiles()
    {
        ...
        try
        {
            m_sandboxFsTree->deserialize( m_flatDrivePaths.m_sandboxFsTreeFile );
        }
        catch (...)
        {
    ...
    void createCatchingUpFileList( const Folder& folder )
    {
        ...
        for( const auto& child : folder.m_childs )
        {
            ...
            else
            {
                ...
                if ( !fs::exists( m_flatDrivePaths.m_driveFolder / toString(hash), err ) )
                {
...
{
    return std::make_shared<ModificationRequestDriveTask>(request);
}
