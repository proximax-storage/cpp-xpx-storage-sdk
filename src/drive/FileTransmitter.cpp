...
namespace sirius { namespace drive {
	...
	private:
		static Hash256 toHash(const lt::sha256_hash& hash) {
			Hash256 finalHash;
		...
		static lt::sha256_hash toLtHash(const Hash256& hash) {
	...
	public:
		...
		Hash256 prepareActionListForUpload(const ActionList&, const std::string&, uint16_t) override {
			return Hash256();
		}
		...
		void download(
			...
			const std::string magnetLink = CreateMagnetLink(hash);
		...
		void addFile(const Key&, const std::string& filePath, ErrorHandler handler)  override {
			lt::add_files(mFileStorage, filePath);
		...
		void removeFile(const Key&, const Hash256&, const std::string&, ErrorHandler)  override {
		}
