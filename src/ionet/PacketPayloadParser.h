...
namespace sirius { namespace ionet {
	...
	template<typename TEntity, typename TIsValidPredicate>
	std::vector<size_t> ExtractEntityOffsets(const utils::RawBuffer& buffer, TIsValidPredicate isValid) {
...
#define CATAPULT_LOG_DATA_SIZE_ERROR(SIZE, MESSAGE) \
	...
	template<typename TEntity, typename TIsValidPredicate>
	bool ContainsSingleEntity(const utils::RawBuffer& buffer, TIsValidPredicate isValid) {
		if (0 == buffer.Size)
			return false;
	...
	size_t CountFixedSizeStructures(const utils::RawBuffer& buffer) {
		if (0 == buffer.Size)
			return 0;
